...
class ClipFinetuner(nn.Module):
    def __init__(self, num_classes, block_dropout_rate=0.0, classifier_dropout_rate=0.0, clip_model="RN50"):
        super().__init__()
        ...
        # Add dropout to the encoder layers with the specified block dropout rate
        self.add_dropout_to_visual_encoder(block_dropout_rate)

        # New classifier layer with separate classifier dropout rate
    ...
    def add_dropout_to_visual_encoder(self, block_dropout_rate):
        # Adding dropout layers to each bottleneck block in ResNet50
        resnet = self.clip_model.visual

        # Insert dropout after each residual block
        for layer_name in ['layer1', 'layer2', 'layer3', 'layer4']:
            layer = getattr(resnet, layer_name)
            for block in layer:
                block.dropout = nn.Dropout(block_dropout_rate)

    def forward(self, image):
        # Forward through ResNet50 with dropout applied after each block
        features = self.clip_model.encode_image(image).float()
        dropout_features = self.classifier_dropout(features)
        return self.classifier(dropout_features)
